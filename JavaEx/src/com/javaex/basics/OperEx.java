...
public class OperEx {
	...
	private static void conditionalOper() {
		...
		passMsg = score >= 80 ? "Good" : score >= 50 ? "Pass" : "Fail";
		
		System.out.println("당신은 " + score + "점으로 " + passMsg + "입니다.");
			
		
		
	}
	// 비트 시프트 연산자
	// 비트 단위 이동 <<: 1번 할 때마다 2승씩 곱 >>: 1번 할 때마다 2승식 나누기
	...
	private static void bitOper() {
		...
		int result = b1 & b2; // 비트 논리 곱: 둘 다 1이어야 1비트로 세팅
	...
	//비교연산자, 논리연산자
	// 결과로 boolean 반환, 논리값으로 프로그램의 흐름 제어 -> 중요
	private static void logicOper() {
		...
		System.out.println("n3가 0 이하 10 이상 영역에 있는가? " + r1orr2);
	...
	private static void arithOperEx() {
		...
		// 실제 나눗셈의 결과는?
		System.out.println((float)a / (float)b);  // int -> float형으로 변환
		System.out.println((float)a / b);  // b는 float로 자동변환(float형이 int형 보다 범위가 넓기 떄문에!)
		...
		System.out.println((float)a / 0); // 실수를 0으로 나누면 ㄱㅊ Infinity
